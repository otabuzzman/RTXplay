...
void SimpleSM::eaCtlAnm() {
	...
	if ( args_.flag_tracesm() ) std::cerr << "from " << stateName[s] << " to "  << stateName[static_cast<int>( next )] << " ... " ;
	{ // perform action
		SmParam* smparam = static_cast<SmParam*>( glfwGetWindowUserPointer( window_ ) ) ;
		// toggle animation
	...
	// transition
	h_state_.push( next ) ;
	if ( args_.flag_tracesm() ) std::cerr << "finished" << std::endl ;
}
...
void SimpleSM::eaReject() {
	...
	if ( args_.flag_tracesm() ) std::cerr << "rejected in " << stateName[s] << std::endl ;
	h_event_.pop() ;
}
